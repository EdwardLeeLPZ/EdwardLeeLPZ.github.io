...
description: Vision-language auto-labeling that turned 1,500 driving episodes into 3D occupancy training labels inside a $5,000 labeling budget, and the vehicle model distilled from them.
category: demo
importance: 1
...
keyframes, and each keyframe is a 200 × 200 × 16 grid of 0.4 m voxels. I built a pipeline that
generates those labels from LiDAR returns, camera video and an open-vocabulary detector, ran it over
1,500 episodes inside a fixed $5,000 labeling budget, found and fixed the geometric defect that made
the first run unusable, rebuilt the corpus on 251 machines in 4.6 hours, and handed the distilled single-frame
model to the vehicle team. This page is about the engineering; the model architecture is published
separately {% cite li2025ago %}.
...
store and the open-vocabulary detector were existing infrastructure. Every dollar below is node-hours
times a price anchor, not an invoice.

## Hand annotation was never an option, so the labels had to be earned

Voxel labels cannot be drawn. One keyframe holds 640,000 cells, and the interesting ones are the
cells no sensor saw directly. The alternative is to run an open-vocabulary 2D detector over the
camera images, paint their classes onto the LiDAR returns that carry the geometry, and aggregate many
frames of one drive into a single grid, so a parked car seen from three angles fills in as a solid
object instead of three shells. Things come from an open-vocabulary detector prompted with a short
phrase list; the background classes come from a closed-set segmenter.

That trade has a price: the detector runs on every camera of every frame, so cost scales with the
corpus, and the budget was fixed. Three constraints followed. The expensive stage could be paid for
only once, so detector output had to be cached and later label revisions had to be CPU jobs. The
pipeline had to be verifiable without ground truth, because there is none. And quality fixes had to
ship behind switches, nine of the ten of them, so a bad idea could be turned off without a re-run.
...
Everything downstream of detection is deterministic, which is what makes a second label version
affordable: the detector output is cached per frame, so rebuilding the corpus never pays the GPU
bill twice.
...
The first 700-episode run had a specific, repeatable defect: the fence class smeared flat across
distant ground, and voxels floating above the lane where nothing was. Two rounds of plausible fixes,
depth gating on lifted classes, solid box fill and geometric carving, cut the defect by 72 % and were
still rejected on review, because the worst frames barely moved.
...
±40 m grid is at most 57 m from that frame's car. The condemned frame went from 1,451 ghost voxels to 3. Across the audit package the count fell 87.4 % cumulatively, while near-range vehicle voxels rose
0.07 %, the guard that shows the cleanup did not eat real content.
...
The chart below is cost per episode at each measurement point. The two fleet bars are the ones that
count, because they are whole-campaign actuals rather than single-episode probes.
...
Figure 3: the two labeling waves, 700 episodes at $2.84 and 800 at $2.87, come to $4,284; about $600
more went on probes, the three fix rounds and the wave-1 re-aggregation, bringing the labeling pool
to roughly $4.9k of the $5,000 ceiling. The bars are not interchangeable: the fleet bars include GPU detection, the
second-version bar is CPU-only re-aggregation from caches already paid for, and the probes are single
episodes.
...
evening kept the budget plan off a number that was not real.

The second label version then ran as 251 CPU shards launched in one 27-minute window: 1,500 episodes,
30,000 keyframe files, 4.57 hours wall clock, 251 of 251 shards succeeded, no retries. Idempotent
shard boundaries and done-markers are what made zero failures survivable rather than lucky.

## Seventy percent of every training step was a frozen teacher

Before launching a full training run I profiled the step. The frozen vision-language teacher that
produces the distillation targets took 2.202 s of 3.155 s, 69.8 % of the step, and it recomputed the
same targets for the same frames every epoch.
...
Figure 4: precomputing the targets once for all 30,000 samples, then reading them from cache in
...
The same instinct paid off inside the label pipeline. The free-space rule I needed, mark a voxel free
only when an unobstructed line runs from it back to some LiDAR pose along the drive, was a
161-second-per-frame ray march. Rewriting it
as a depth-buffer test from the sensor's own viewpoint made it 8.5 seconds, 19× in the cloud A/B and
21.6× in a local measurement over 32 origins, with zero fabricated free voxels and 98.8 % of the
exact method's free space kept. The rule itself was checked separately on held-out frames: it
mislabels 0.11 % of the free voxels it adds, against 2.18 % for the forward ray-casting method it
replaced.

## Rebuilding the labels changed what the model learned

The second label version was not a bug fix. The first corpus supervised about 31 % of the forward
half of each grid; the rest was unobserved, contributed no gradient, and left the model free to
hallucinate there. Drag the slider: occupancy is essentially unchanged, and what moves is free space
filling in the sensor shadows.
...
Figure 5: one keyframe, first version left, second version right, bird's-eye view above and a
vertical slice below. Occupied voxels stay at 9.7 % of the forward half-grid across the two
versions; unobserved falls from 59.6 % to 17.9 % and free space rises from 30.7 % to 72.4 %.
Supervision coverage over the two labeling waves rose from about 31 % to 77.1 %.

Retraining on the second version collapsed two geometric artifacts no aggregate metric had flagged: a
membrane of occupied voxels along the top of the grid, and a wall of enriched occupancy at the
lateral edges. Over 11 episodes the ceiling-to-interior ratio fell from a median of 5.20 to 0.72, and
to 0.78 on the larger 150-episode split; edge enrichment fell from 1.98 to 1.08. Both improved in 11
of 11 episodes.
...
Figure 6: each line is one episode, the thick line is the median. Two checkpoints must be
compared on one exam rather than two, so the table below scores both on the same nine frames against
the same label version. Absolute values are small and the labels they are scored
against are themselves generated; the directions are what the table is for.
...
which 476 MB was optimizer and scheduler state. All the model still needs from the
vision-language stack is a frozen 21 × 512 table of text embeddings, small enough to ship as a file,
so nothing imports that stack at runtime. The slim checkpoint's predicted grids are bit-identical to
the full one's.

The handoff document leads with the limits, and so does this page. The model is single-frame and
covers the forward 180° only. Its roughly 20 s per frame is a CPU debugging figure, not a deployment
latency. It has had no safety validation of any kind, and the vulnerable-road-user classes are not
dependable at this scale. The ghost result is an acceptance on a 19-episode package, not a proof over
1,500. The ceiling membrane was reduced, not eliminated: at 0.78 it still sits above the labels' own
0.53. The lateral wall was fixed on the label side, and the training-side prior I tried for it was a
negative result that cost a full 24-epoch run. And the corpus is not internally uniform, because a
cache-replay path rewrote some stuff-class assignments between versions, so the training set mixes
two conventions. That is a known debt, not a solved problem.

## What I would keep

Trace a suspicious number to a physical mechanism before fixing anything: 163.835 m is a data type,
not a distance, and two rounds of reasonable fixes cleared most of the aggregate count while leaving
the frames under review untouched, which is how a misdiagnosis hides inside a good average. Put
a guard metric beside every cleanup, because "the defect went away" and "the content survived" are
different claims. And audit your own good news hardest, because the 5.69× I retracted would have been
